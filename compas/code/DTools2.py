...
class DTools():
    ...
    def setFeatures(self,D=[],X=[],Y=[]):
        ...
        self.dfD_to_Y_address = pd.DataFrame(range(len(list(self.dfMask_Pxyd_to_Pyd))),index=self.dfMask_Pxyd_to_Pyd.columns)
        self.dfD_to_Y_address = pd.pivot_table(self.dfD_to_Y_address.reset_index(),columns=self.D_features,index=self.Y_features,values=0)
    ...
    def optimize(self,epsilon=1.,dlist=[],verbose=True,solver=None,cost='TV'):

    	# dlist will be collection of variables D and distortion metric in the form (category, distortion). Note that if D is more than one feature, *all* features must be specified.

    	# for now we assume the same function across all categories
    	# and the same distortion constraint per discriminatory category
        ...
        PYhD = Variable(self.dfD_to_Y_address.shape[1],self.dfD_to_Y_address.shape[0])
        ...
        #     constraints.append(sum_entries(mul_elemwise(self.CMlist[i],Pxy_xhyh),axis=1)<=self.dlist[i])

        t = self.DXY_index.tolist()
        for (cat,mean_distortion) in dlist:
        	# get index of category in dfP
        	relev_index = [i for i in range(len(t)) if set(cat) < set(t[i])]

        	# add constraint and sum across axis
        	constraints.append(sum_entries(mul_elemwise(self.Dmatrix,Pmap[relev_index,:]),axis=1)<=mean_distortion)
        ...
        #constraints.append(sum_entries(mul_elemwise(self.dfD.as_matrix(),Pxy_xhyhJoint),axis=1)<=self.mean_distortion)
        ...
        if cost=='TV':
        	obj = Minimize(norm(PXhYh-PxyMarginal, 1)/2.0)
        elif cost == 'L2':
        	obj = Minimize(norm(PXhYh-PxyMarginal, 2))
        else:
        	non_zero_ix = list(np.where(PxyMarginal>1e-5)[0])
	        obj = Minimize(sum(kl_div(PxyMarginal[non_zero_ix],PXhYh[non_zero_ix])))

        prob = Problem(obj,constraints)
        if solver == 'SCS':
        	prob.solve(verbose=verbose,solver=solver,max_iters = 75000)
        else:
        ...
        for (cat,mean_distortion) in dlist:
        	...
        	# add constraint and sum across axis
        	self.const.append((cat,sum_entries(mul_elemwise(self.Dmatrix,Pmap[relev_index,:]),axis=1).value))

    def computeMarginals(self):
        #mask = self.dfMask_Pxyd_to_Pxy.T.values
        self.dfFull = pd.DataFrame((np.diag(self.PxydMarginal)).dot(self.dfP.values),index = self.dfP.index, columns=self.dfP.columns)

        self.dfPyMarginal = pd.DataFrame(self.PxydMarginal.dot(self.dfMask_Pxyd_to_Py).T,index=self.dfMask_Pxyd_to_Py.columns)
        self.dfPdMarginal = pd.DataFrame(self.PxydMarginal.dot(self.dfMask_Pxyd_to_Pd).T,index = self.dfMask_Pxyd_to_Pd.columns)
